...
then
    check_ass=0;
    echo "Checking all parts"
else
    check_ass=$1
    echo "Checking part " $check_ass
fi


if [ $check_ass -eq 0 ] || [ $check_ass -eq 1 ]
then
...
timeout 2 ./triang 11 | wc -l | grep 11 || exit 1
...
if [ $check_ass -eq 0 ] || [ $check_ass -eq 2 ]
then
...
if [ $check_ass -eq 0 ] || [ $check_ass -eq 3 ]
then
